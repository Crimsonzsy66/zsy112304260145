...
def preprocess_reviews(reviews: Iterable[str], *, remove_stopwords: bool) -> list[list[str]]:
    return [
        tokenize_text(review, remove_stopwords=remove_stopwords)
...
def preprocess_for_tfidf(reviews: Iterable[str]) -> list[str]:
    return [
        re.sub(r"[^a-zA-Z]", " ", strip_html(expand_contractions(str(review))).lower())
        for review in reviews
    ]
...
def build_average_vectors(tokenized_reviews: list[list[str]], model: Word2Vec) -> np.ndarray:
    ...
    for idx, tokens in enumerate(tokenized_reviews):
        ...
        if valid_tokens:
            features[idx] = np.mean(vocab[valid_tokens], axis=0)
    return features
...
def evaluate_with_cv(
    ...
    for fold_idx, (train_idx, valid_idx) in enumerate(splitter.split(features, labels), start=1):
        ...
        print(f"Fold {fold_idx}/{folds}: ROC-AUC = {auc:.5f} (Time: {fold_time:.2f}s)")

    avg_time = np.mean(fold_times)
    print(f"Mean ROC-AUC: {float(np.mean(scores)):.5f} (std={np.std(scores):.5f}, avg time: {avg_time:.2f}s)")
...
def validate_submission(df: pd.DataFrame, test_df: pd.DataFrame) -> bool:
    if df.shape[0] != test_df.shape[0]:
        logger.error(f"Submission row count mismatch: {df.shape[0]} vs {test_df.shape[0]}")
        return False

    if df.shape[1] != 2:
        logger.error(f"Submission should have 2 columns, got {df.shape[1]}")
        return False

    if not all(df.columns == ['id', 'sentiment']):
        logger.error(f"Invalid columns: {df.columns.tolist()}")
        return False

    if df['sentiment'].isnull().any():
        logger.error("Found null values in sentiment predictions")
        return False

    if not ((df['sentiment'] >= 0) & (df['sentiment'] <= 1)).all():
        logger.error("Sentiment scores must be in range [0, 1]")
        return False

    missing_ids = set(test_df['id']) - set(df['id'])
    if missing_ids:
        logger.error(f"Missing IDs in submission: {len(missing_ids)}")
        return False
...
def build_submission_path(timestamp: str, feature_name: str, classifier_name: str, lr_c: float = None, rf_trees: int = None) -> Path:
    SUBMISSION_DIR.mkdir(parents=True, exist_ok=True)
    safe_timestamp = timestamp.replace(":", "-")
    safe_feature_name = re.sub(r"[^a-zA-Z0-9_]+", "_", feature_name)

    if classifier_name == "random_forest":
        lr_c_str = f"trees{rf_trees}"
    else:
        lr_c_str = f"c{lr_c}"

    filename = f"submission_{safe_timestamp}_{classifier_name}_{lr_c_str}_{safe_feature_name}.csv"
...
def main() -> None:
    try:
        ...
        logger.info(
            ...
            f"test rows: {len(test_df)} (loaded in {data_load_time:.2f}s)"
        )
        ...
        classifier_label = CLASSIFIER_LABELS[args.classifier]

        if args.feature == "tfidf":
            logger.info("Using TF-IDF with N-gram features")
            logger.info("Preprocessing text (keeping negation words)...")
            ...
            logger.info(f"Building TF-IDF features with n-gram ({args.ngram_min}-{args.ngram_max})...")
            feature_start = time.time()
            train_features, test_features, vectorizer = build_tfidf_features(
                train_texts, test_texts, args
            )
            feature_time = time.time() - feature_start
            logger.info(f"TF-IDF feature shape: {train_features.shape} (built in {feature_time:.2f}s)")

            feature_name = f"tfidf_ngram{args.ngram_min}-{args.ngram_max}_max{args.max_features}"
            selected_name = feature_name
            selected_auc = None
            logger.info(f"Evaluating {feature_name} + {classifier_label}...")
            cv_start = time.time()
            ...
            logger.info(f"TF-IDF ROC-AUC: {mean_auc:.5f} (CV completed in {cv_time:.2f}s)")
            ...
            logger.info(f"Final model trained in {train_time:.2f}s")

        else:
            logger.info("Using Word2Vec features")
            logger.info("Tokenizing reviews for Word2Vec training...")
            ...
            logger.info("Loading or training Word2Vec...")
            ...
            logger.info(f"Word2Vec {action}: {cache_path}")
            logger.info(f"Word2Vec vocabulary size: {len(model.wv)}")

            logger.info("Tokenizing labeled and test reviews for feature construction...")
            ...
            logger.info(f"Evaluating mean Word2Vec + {classifier_label}...")
            ...
            logger.info(f"Mean embeddings ROC-AUC: {mean_auc:.5f} (std={np.std(mean_scores):.5f})")
            ...
            logger.info(f"Clustering word vectors with KMeans (k={args.kmeans_clusters})...")
            ...
            logger.info(f"Evaluating bag-of-centroids + {classifier_label}...")
            ...
            logger.info(f"KMeans centroid features ROC-AUC: {kmeans_auc:.5f} (std={np.std(kmeans_scores):.5f})")
            ...
            logger.info(f"Selected feature set: {selected_name} (CV ROC-AUC = {selected_auc:.5f})")
        ...
        submission_path = build_submission_path(
            run_timestamp,
            selected_name,
            args.classifier,
            args.lr_c,
        ...
        logger.info(f"Saved submission to: {submission_path} ({submission_time:.2f}s)")
        logger.info(f"Submission shape: {submission_df.shape}")
        logger.info(f"Sentiment score range: [{submission_scores.min():.4f}, {submission_scores.max():.4f}]")

        if not validate_submission(submission_df, test_df):
            logger.warning("Submission validation failed! Please check the file.")

        log_attempt(
            timestamp=run_timestamp,
            feature_type=f"{selected_name}_submission",
            scores=[],
            mean_auc=selected_auc if selected_auc is not None else mean_auc,
            args=args,
            labeled_rows=len(labeled_df),
            unlabeled_rows=len(unlabeled_df),
            submission_path=submission_path,
        )
        logger.info(f"Updated attempt log at: {ATTEMPT_LOG_PATH}")

        overall_time = time.time() - overall_start
        logger.info(f"Training pipeline completed in {overall_time:.2f}s ({overall_time/60:.2f} minutes)")
        logger.info(f"Best CV ROC-AUC: {selected_auc if selected_auc is not None else mean_auc:.5f}")
